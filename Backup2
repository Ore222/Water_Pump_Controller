...
#define _pumppausedelay 240	// pump pause monitoring delay approximate time (seconds, times 2).
...
void waterleveldisp()
			{	
				...
				if (_highsensor == HIGH && _lowsensor == HIGH && _dispflagfull == HIGH)
					{ // water level is on top sensor
						LCD_DisplayString("Tank Water: Full");
						_dispflagfull = LOW;
						_dispflaglow = HIGH;
						_dispflagokay = HIGH;
						_dispflagerr = HIGH;
				...
				if (_highsensor == LOW && _lowsensor == HIGH && _dispflagokay == HIGH)
					{ // water level is below high sensor
						LCD_DisplayString("Tank Water: Okay");
  						_dispflagokay = LOW;
						_dispflaglow = HIGH;
						_dispflagfull = HIGH;
						_dispflagerr = HIGH;
				...
				if (_highsensor == HIGH && _lowsensor == LOW && _dispflagerr == HIGH)
					{ // sensor malfunction
						LCD_DisplayString("Tank Water: Err.");
		  				_dispflagerr = LOW;
						_dispflaglow = HIGH;
						_dispflagfull = HIGH;
						_dispflagokay = HIGH;
				...
				if (_highsensor == LOW && _lowsensor == LOW && _dispflaglow == HIGH)
					{ // water level is below low sensor
						LCD_DisplayString("Tank Water: Low!");
		  				_dispflaglow = LOW;
						_dispflagokay = HIGH;
						_dispflagfull = HIGH;
						_dispflagerr = HIGH;
...
void status_manualidle()
		{
			...
			if (_manualidledispflag == HIGH)
			{
				...
				if (_highsensor == HIGH)
				{
					if (_pumponflag == HIGH || _stopdispflag == LOW)
						{
							LCD_DisplayString("----------------");
						}
				}

				else if (_highsensor != HIGH && _pumponflag == HIGH)
				{  												
...
void status_startpump()
	{
		waterleveldisp();				// start water pump when START button is pressed
		_relay = ON;				
		_pumponflag = LOW;
		_stopdispflag = HIGH;
		_autoidledispflag = HIGH;
...
void status_pausepump()
	{
		...
		count = 0;
		LCD_GoToLineOne();
		LCD_DisplayString("Water Flow Error");
		LCD_GoToLineTwo();
